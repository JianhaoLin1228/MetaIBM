...
运行方式（建议在项目根目录执行）：
    python test/test_patch_recombination_smoke.py
"""
...
def build_patch() -> tuple[patch, object, individual, individual]:
    p = patch('p0', 0, (0, 0))
    p.add_habitat(
        hab_name='h0',
        hab_index=0,
    ...
    return p, h, female, male


def _print_pool(label: str, pool):
    print('\n' + '=' * 72)
    print(label)
...
def main() -> int:
    ...
    # 1) patch_sexual_birth_germinate
    p1, h1, f1, m1 = build_patch()
    _clear_trace(f1); _clear_trace(m1)
    c1 = p1.patch_sexual_birth_germinate(
        1.0, 0.0, [0.0, 0.0],
        recomb_method='multi_crossover',
        recomb_rate=0.3
    )
    print('\n' + '=' * 72)
    print('Case 1: patch_sexual_birth_germinate')
    print(f'  germinated count = {c1}')
    print(f'  patch individual num = {p1.get_patch_individual_num()}')
    _print_trace('  female parent traces', f1)
    _print_trace('  male parent traces', m1)

    # 2) patch_mixed_birth_germinate
    p2, h2, f2, m2 = build_patch()
    _clear_trace(f2); _clear_trace(m2)
    c2 = p2.patch_mixed_birth_germinate(
        1.0, 1.0, 0.0, [0.0, 0.0],
        recomb_method='multi_crossover',
        recomb_rate=0.3
    )
    print('\n' + '=' * 72)
    print('Case 2: patch_mixed_birth_germinate')
    print(f'  germinated count = {c2}')
    print(f'  patch individual num = {p2.get_patch_individual_num()}')
    _print_trace('  female parent traces', f2)
    _print_trace('  male parent traces', m2)

    # 3) patch_sex_reproduce_mutate_into_offspring_pool
    p3, h3, f3, m3 = build_patch()
    _clear_trace(f3); _clear_trace(m3)
    n3 = p3.patch_sex_reproduce_mutate_into_offspring_pool(
        1.0, 0.0, [0.0, 0.0],
        recomb_method='multi_crossover',
        recomb_rate=0.3
    )
    _print_pool('Case 3: patch_sex_reproduce_mutate_into_offspring_pool', p3.get_patch_offspring_pool())
    print(f'  returned count = {n3}')
    _print_trace('  female parent traces', f3)
    _print_trace('  male parent traces', m3)

    # 4) patch_mix_reproduce_mutate_into_offspring_pool
    p4, h4, f4, m4 = build_patch()
    _clear_trace(f4); _clear_trace(m4)
    n4 = p4.patch_mix_reproduce_mutate_into_offspring_pool(
        1.0, 1.0, 0.0, [0.0, 0.0],
        recomb_method='multi_crossover',
        recomb_rate=0.3
    )
    _print_pool('Case 4: patch_mix_reproduce_mutate_into_offspring_pool', p4.get_patch_offspring_pool())
    print(f'  returned count = {n4}')
    _print_trace('  female parent traces', f4)
    _print_trace('  male parent traces', m4)

    print('\nPASS: patch.py modified methods test passed')
    return 0
...
if __name__ == '__main__':
    try:
        ...
        print('FAIL: patch.py modified methods test failed')
